...
class BatchNorm: # dims exclude N, eg. dims = (C, h, w)
	def __init__(self, dims, alpha = 0.1, eps = 1e-3, dtype = np.float32):
		...
		self.gamma = np.ones((1, *dims), dtype = self.dtype)
		self.beta = np.zeros((1, *dims), dtype = self.dtype)
		...
		self.mu_moving = np.zeros((1, *dims), dtype = self.dtype)
		self.var_moving = np.zeros((1, *dims), dtype = self.dtype)
